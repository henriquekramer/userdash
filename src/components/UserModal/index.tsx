...
export function UserModal({ initialValues }: UserModalProps){
  ...
  const { register, handleSubmit, watch, setValue, formState: { errors } } = useForm({ defaultValues: initialValues, resolver: yupResolver(validationSchema) })
        ...
        <form onSubmit={handleSubmit(onSubmit, onError)}>
          <main>
            ...
            <div>
              ...
              <input type="text" id="email" {...register('email')} placeholder="Ex: email@exemplo.com"/>
              {errors?.email?.type && <InputError type={errors.email.type} field="email"/>}
            </div>
            <div>
              <label htmlFor="phone">Celular</label>
              <input type="tel" id="phone" {...register('phone')} placeholder="(99) 99999-9999"/>
            ...
            <div>
              ...
              <input type="text" id="state" {...register('state')} placeholder="Ex: Paraná"/>
              {errors?.state?.type && <InputError type={errors.state.type} field="state"/>}
            </div>
            <div>
              <label htmlFor="street">Rua *</label>
              <input type="text" id="street" {...register('street')} placeholder="Ex: Rua Paraná"/>
              {errors?.street?.type && <InputError type={errors.street.type} field="street"/>}
            </div>
            <div>
              <label htmlFor="number">Número *</label>
              <input type="text" id="number" {...register('number')} placeholder="Ex: 250"/>
              {errors?.number?.type && <InputError type={errors.number.type} field="number"/>}
            </div>
            <div>
              <label htmlFor="avatar">Foto de perfil(link)</label>
              <input type="text" id="avatar" {...register('avatar')} placeholder="Ex: https://avatar..."/>
            </div>
          </main>
          <footer>
            <button type='submit'>
